...
def rag_database_earnings_call(
    ...
    def query_database_earnings_call(question: str, quarter: str) -> str:
        """
        此工具將針對給定的問題和季度查詢財報電話會議記錄數據庫，
        並從財報電話會議中檢索相關文本以及處理相關文件的發言者。
        此工具有助於回答財報電話會議記錄中的問題。
...
def rag_database_sec_filings(
    ...
    if data_source == 'unstructured':
        ...
        sec_filings_db = Chroma.from_documents(
        ...
        def query_database_sec_filings(question: str, form_name: str) -> str:
            """
            此工具將針對給定的問題和表格名稱查詢 SEC 文件數據庫，
            並從 SEC 文件中檢索相關文本。此工具有助於回答 SEC 文件中的問題。
        ...
        def query_database_sec_filings(question: str) -> str:
            """
            此工具將針對給定的問題查詢 SEC 文件數據庫，
            並從 SEC 文件中檢索相關文本。此工具有助於回答 SEC 文件中的問題。
            ...
            for i, doc in enumerate(docs):
                result += f"\n\n文件 {i+1}：\n"
                result += doc.page_content
                result += f"\n\n文件名稱：{doc.metadata.get('file_name', '未知')}"
        ...
        raise ValueError(f"不支援的數據來源：{data_source}")


def rag_database_markdown_sec(
        ticker: str,
        year: str,
        filing_types: List[str] = ["10-K", "10-Q"],
        include_amends: bool = True
    ...
    """
    建立基於 Markdown 的 SEC 文件 RAG 數據庫並返回查詢函數和表格名稱
    """
    # 首先獲取 unstructured 數據以獲得表格名稱
    sec_data, sec_form_names = get_data(
        ticker=ticker, 
        year=year,
    ...
    try:
        ...
        print(f"讀取 Markdown 檔案時發生錯誤：{e}")
        return None, sec_form_names
    ...
    def query_database_markdown_sec(question: str, sec_form_name: str) -> str:
        """
        此工具將針對給定的問題和表格名稱查詢 SEC 文件數據庫，
        並從 SEC 文件中檢索相關文本和章節名稱。此工具有助於回答 SEC 文件中的問題。

        參數：
        question (str)：要查詢數據庫以獲取相關文件的問題。
        sec_form_name (str)：問題中討論的 SEC 表格名稱。可以是 10-K（年度數據）或 10-Q（季度數據）。
                             對於季度數據，可以是 10-Q2 表示第二季度，其他季度類似。
