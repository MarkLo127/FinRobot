...
def pipeline_api(
    ...
    """支援許多章節，包括：RISK_FACTORS、MANAGEMENT_DISCUSSION 等等"""
    ...
    if sec_document.filing_type not in VALID_FILING_TYPES:
        raise ValueError(
            ...
            f"必須是下列其中之一：{','.join(VALID_FILING_TYPES)}"
        )
    results = {}
    ...
    if response_type == "application/json":
        if response_schema == LABELSTUDIO:
            ...
            raise ValueError(
                f"不支援 {response_type} 的 output_schema '{response_schema}'"
            )
        ...
        raise ValueError(f"不支援的回應類型 '{response_type}'")


def get_validated_mimetype(file):
    """
    傳回檔案的 mimetype，可透過 file.content_type 或 mimetypes 函式庫 (如果前者太通用) 取得。如果使用者已設定 UNSTRUCTURED_ALLOWED_MIMETYPES，則會根據此清單進行驗證，並針對無效類型傳回 HTTP 400。
    """
    ...
    if isinstance(text_files, list) and len(text_files):
        if len(text_files) > 1:
            if content_type and content_type not in [
                ...
                raise HTTPException(
                    detail=(
                        f"媒體類型 {content_type} 與"
                        '回應類型 "multipart/mixed" 衝突。\n'
                    ),
