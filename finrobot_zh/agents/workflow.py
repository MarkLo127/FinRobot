...
class finrobot_zh(AssistantAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(agent_config, str):
            ...
            assert name in library, f"在代理程式庫中找不到 finrobot_zh {name}。"
            agent_config = library[name]

        agent_config = self._preprocess_config(agent_config)

        assert agent_config, f"需要提供代理配置。"
        assert agent_config.get("name", ""), f"配置中需要包含名稱。"
...
class SingleAssistantShadow(SingleAssistant):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.assistant_shadow = finrobot_zh(
            agent_config,
            toolkits=[],
            llm_config=llm_config,
            proxy=None,
...
class MultiAssistantBase(ABC):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert self.agent_configs, f"需要提供代理配置。"
        self.agents = []
        self._init_agents()
...
class MultiAssistant(MultiAssistantBase):
    ...
    def _get_representative(self):
        def custom_speaker_selection_func(
            ...
            """定義自定義發言者選擇函數。
            建議的方法是為群組聊天中的每個發言者定義一個轉換。

            返回：
                返回一個 `Agent` 類或從 ['auto', 'manual', 'random', 'round_robin'] 中選擇一個預設方法使用。
            """
...
class MultiAssistantWithLeader(MultiAssistantBase):
    """
    基於領導者的工作流程，通過巢狀聊天將多個代理連接到一個領導者代理。
