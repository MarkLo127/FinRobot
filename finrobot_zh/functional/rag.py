...
PROMPT_RAG_FUNC = """以下是根據您的查詢從所需檔案中檢索到的內容。
如果您無法根據當前內容回答問題（無論是否有內容），您應該根據需求嘗試使用更精確的搜索查詢，或請求更多內容。
...
def get_rag_function(retrieve_config, description=""):
    ...
    rag_assitant = RetrieveUserProxyAgent(
        ...
        description="擁有額外內容檢索能力以解決複雜問題的助手。",
    )

    def retrieve_content(
        message: Annotated[
            str,
            "保持原意並且可用於從提供的檔案中檢索內容以進行程式碼生成或問題回答的精確查詢訊息。"
            "例如：'利潤率的同比比較'、'NVIDIA Q4 的風險因素'、'使用 YFinance 檢索歷史股價數據'",
        ],
        n_results: Annotated[int, "要檢索的結果數量，預設為 3"] = 3,
    ) -> str:

        rag_assitant.n_results = n_results  # 設置要檢索的結果數量
        # 檢查是否需要更新內容
    ...
    if description:
        ...
        retrieve_content.__doc__ = "從文件中檢索內容以協助問題回答或程式碼生成。"
