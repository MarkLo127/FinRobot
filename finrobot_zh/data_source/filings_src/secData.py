...
import re
from finrobot_zh.data_source.filings_src.sec_filings import SECExtractor
import concurrent.futures
from functools import partial
from finrobot_zh.data_source.filings_src.prepline_sec_filings.fetch import get_cik_by_ticker
import requests
from finrobot_zh.data_source.filings_src.prepline_sec_filings.fetch import get_filing
import pandas as pd
...
def sec_main(
    ticker: str,
    year: str,
    filing_types: List[str] = ["10-K", "10-Q","20-F"],
    include_amends=True,
):
    ...
    if response.status_code == 200:
        ...
        print(f"錯誤：無法獲取資料。狀態代碼：{response.status_code}")
    ...
    assert len(results_texts) == len(
        acc_nums_list
    ), f"抓取的文本數量 {len(results_texts)} 與收件編號文本數量 {len(acc_nums_list)} 不符"
    print("抓取完成")
    print("開始提取資料")
    ...
    assert len(section_texts) == len(
        acc_nums_list
    ), f"章節文本數量 {len(section_texts)} 與收件編號文本數量 {len(acc_nums_list)} 不符"
