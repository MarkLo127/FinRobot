...
from finrobot_zh.data_source.filings_src.prepline_sec_filings.sections import SECSection
...
REPORT_TYPES: Final[List[str]] = ["10-K", "10-Q", "10-K/A", "10-Q/A","20-F","20-F/A"]
...
class SECDocument(HTMLDocument):
    ...
    def _is_last_section_in_report(
        ...
        if self.filing_type in ["10-K", "10-K/A", "20-F", "20-F/A"]:
...
def is_20f_item_title(title: str) -> bool:
    """Determines if a title corresponds to a 10-K item heading."""
    return ITEM_TITLE_RE.match(clean_sec_text(title, lowercase=True)) is not None

def is_10k_risk_title(title: str) -> bool:
    """Checks to see if the title matches the pattern for the risk heading."""
    return ("1a" in title.lower() or "3d" in title.lower() or "risk factors" in title.lower()) and not (
        "summary" in title.lower()
    )

def is_20f_risk_title(title: str) -> bool:
    """Checks to see if the title matches the pattern for the risk heading."""
    return ("1a" in title.lower() or "3d" in title.lower() or "risk factors" in title.lower()) and not (
        "summary" in title.lower()
    )
...
def match_20f_toc_title_to_section(text: str, title: str) -> bool:
    """Matches a 20-F style title from the table of contents to the associated title in the document
    body"""
    if re.match(ITEM_TITLE_RE, title):
        return text.startswith(title)
    else:
